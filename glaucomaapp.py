...
.sample-card:hover { border-color: #C8FF00; }
.sample-card.active { border-color: #C8FF00; background: #141a00; }
.sample-card-label {
    font-family: 'JetBrains Mono', monospace;
    font-size: 0.62rem;
    color: #C8FF00;
...
def load_sample_bytes(path: str) -> bytes | None:
    """Baca sample image dari disk, return bytes atau None jika tidak ada."""
...
if nav == "Detection":
    ...
    # Cek ketersediaan file sample
    samples_available = [s for s in SAMPLE_IMAGES if os.path.exists(s["path"])]

    if samples_available:
        # Session state untuk track pilihan sample
        if "selected_sample" not in st.session_state:
            st.session_state.selected_sample = None

        btn_cols = st.columns(len(SAMPLE_IMAGES))
        for col, smp in zip(btn_cols, SAMPLE_IMAGES):
            with col:
                ...
                if st.button(
                    btn_label,
                    key=f"btn_{smp['id']}",
                    disabled=not file_ok,
                    use_container_width=True,
                    type="primary" if st.session_state.selected_sample == smp["id"] else "secondary",
                ):
                    if st.session_state.selected_sample == smp["id"]:
                        # Toggle off jika klik lagi
                        st.session_state.selected_sample = None
                    else:
                        st.session_state.selected_sample = smp["id"]
                        # Auto-set quality score hint di sidebar tidak bisa langsung,
                        # tapi kita simpan di session state untuk info
                        st.session_state.sample_qs_hint = smp["qs_hint"]
                    st.rerun()

                # Info card di bawah tombol
                status_color = "#4ADE80" if file_ok else "#333"
                tag_color = "#F87171" if "GON+" in smp["tag"] else "#4ADE80"
                st.markdown(f"""
                <div style="
                    border: 1px solid #1E1E1E;
                    border-radius: 4px;
                ...
                ">
                    <div style="font-family:'JetBrains Mono',monospace; font-size:0.6rem;
                                color:{tag_color}; letter-spacing:0.1em; margin-bottom:0.2rem;">
                        {smp['tag']}
                    </div>
                    <div style="font-family:'JetBrains Mono',monospace; font-size:0.58rem; color:#444;">
                        {smp['desc']}
                    </div>
    ...
    # ── Resolve image source: upload > sample ─────────────────────
    image_bytes  = None
    source_label = None
    ...
    if uploaded:
        ...
        # Reset sample pilihan jika user upload manual
        st.session_state.selected_sample = None
